...
httpsServer.on('secureConnection', (tlsSocket) => {
  try {
    const proto = tlsSocket.getProtocol && tlsSocket.getProtocol();
    const cipher = tlsSocket.getCipher && tlsSocket.getCipher();
    const alpn = tlsSocket.alpnProtocol;
    const sni = tlsSocket.servername || tlsSocket.servername === undefined ? tlsSocket.servername : undefined;
    console.log('SECURE_CONNECTION from', tlsSocket.remoteAddress, 'proto=', proto, 'alpn=', alpn, 'cipher=', cipher && cipher.name, 'servername=', sni);
    // log first application data chunk (if any) to see if client sends HTTP bytes
    const onData = (chunk) => {
      try { console.log('FIRST_APP_DATA', chunk && chunk.toString ? chunk.toString('hex').slice(0,400) : chunk); } catch(e) { console.log('FIRST_APP_DATA (unprintable)'); }
      tlsSocket.removeListener('data', onData);
    };
    tlsSocket.once('data', onData);
  } catch (e) { console.error('secureConnection handler error', e); }
});

httpsServer.on('tlsClientError', (err, socket) => {
  console.error('TLS CLIENT ERROR', err && err.stack ? err.stack : err, 'remote=', socket && socket.remoteAddress);
  try { socket.destroy(); } catch(e) {}
});
httpsServer.on('clientError', (err, socket) => {
  console.error('CLIENT ERROR', err && err.stack ? err.stack : err, 'remote=', socket && socket.remoteAddress);
  try { socket.destroy(); } catch(e) {}
});
